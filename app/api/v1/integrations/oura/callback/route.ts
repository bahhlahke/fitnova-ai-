...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (accountRes.error) {
      return jsonError(500, "INTERNAL_ERROR", "Failed to validate Oura callback state.");
    }
    ...
    if (upsertRes.error) {
      return jsonError(500, "INTERNAL_ERROR", "Failed to persist Oura credentials.");
    }

    return NextResponse.redirect(`${appBase}/settings?oura=connected`);
  } catch (error) {
    console.error("oura_callback_unhandled", {
      requestId,
