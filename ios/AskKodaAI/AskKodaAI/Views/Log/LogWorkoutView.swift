...
struct LogWorkoutView: View {
            ...
            if loading && logs.isEmpty {
                ...
                List {
                    Section {
                        ...
                        .onDelete { offsets in
                            Task { await deleteWorkouts(at: offsets) }
                        }
                    }
                }
    ...
    private func deleteWorkouts(at offsets: IndexSet) async {
        guard let ds = dataService else { return }
        for index in offsets.reversed() {
            guard index < logs.count, let logId = logs[index].log_id, !logId.isEmpty else { continue }
