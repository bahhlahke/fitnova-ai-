...
extension KodaAPIService {
    ...
    func aiRetentionRisk(localDate: String? = nil) async throws -> RetentionRiskResponse {
        let body = (localDate.map { ["localDate": $0] }) ?? [:]
    ...
    /// GET /api/v1/spotify/token — returns Spotify access token if user has linked Spotify (OAuth). Nil if not connected.
    func spotifyToken() async throws -> String? {
        struct SpotifyTokenResponse: Decodable { let token: String? }
        do {
            let res: SpotifyTokenResponse = try await get("api/v1/spotify/token")
            return res.token
        } catch let err as KodaAPIError {
            if case .http(404, _) = err { return nil }
            throw err
        } catch let other {
            throw other
        }
